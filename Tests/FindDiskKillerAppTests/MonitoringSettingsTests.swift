...
@Test func applicationReopenRestoresTheMainWindowAndRegularActivationPolicy() throws {
    let root = FileManager.default.temporaryDirectory
        .appending(path: UUID().uuidString, directoryHint: .isDirectory)
    defer { try? FileManager.default.removeItem(at: root) }
    let previousMenuBarOnlyValue = UserDefaults.standard.object(
        forKey: AppActivationPolicy.menuBarOnlyModeKey
    )
    let previousActivationPolicy = NSApplication.shared.activationPolicy()
    UserDefaults.standard.removeObject(forKey: AppActivationPolicy.menuBarOnlyModeKey)
    defer {
        if let previousMenuBarOnlyValue {
            UserDefaults.standard.set(
                previousMenuBarOnlyValue,
                forKey: AppActivationPolicy.menuBarOnlyModeKey
            )
        } else {
            UserDefaults.standard.removeObject(forKey: AppActivationPolicy.menuBarOnlyModeKey)
...
@Test func menuBarOnlyModeKeepsAccessoryPolicyWhenPresentingAndReopening() throws {
    ...
    defer { defaults.removePersistentDomain(forName: suiteName) }
    let previousActivationPolicy = NSApplication.shared.activationPolicy()
    defer { NSApplication.shared.setActivationPolicy(previousActivationPolicy) }
